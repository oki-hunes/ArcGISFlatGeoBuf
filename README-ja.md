...
- **多言語対応**: UI 文字列を英語・日本語で切り替え可能です（`Config.daml` / `Config.ja.daml`、`Strings.resx` / `Strings.ja.resx`）
...
- ArcGIS Pro SDK for .NET 3.7 以降（`D:\Program Files\ArcGIS\Pro\bin\` にインストール済み）
